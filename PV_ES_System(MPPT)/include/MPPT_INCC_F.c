...
//	    	v->DeltaI = 0;
			...
			{
	    	 	v->Cond = v->Ipv / v->Vpv;//v->Cond = v->Ipv*__einvf32(v->Vpv);					/*Cond =-(v->Ipv<<10)/v->Vpv;*/
	    	 	v->IncCond = v->DeltaI / v->DeltaV;//v->IncCond = v->DeltaI*__einvf32(v->DeltaV);			/*IncCond =(DeltaI<<10)/DeltaV;*/
